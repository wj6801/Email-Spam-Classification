...
output: pdf_document
---
Contributions: All three authors, Sungjin Choi, WonJae Lee, and Justin Huang, collaborated closely and contributed equally to the project, putting in the same level of effort and working together to achieve the final result.
...
error.scaled.test = mean(pred != stdtest$V58 ) 
...
error.scaled.train = mean(pred.train != stdtrain$V58 ) 
...
# Standardized
## Linear
svm.train.scaled = svm(V58 ~., data = stdtrain, kernal = "linear", scaled = False, type="C-classification")
svm.train.predict.scaled = predict(svm.train.scaled, stdtrain)
svm.test.predict.scaled = predict(svm.train.scaled, stdtest)

### Classification errors
linear_svm_error_scaled_train <- mean(svm.train.predict.scaled != stdtrain$V58)
linear_svm_error_scaled_test <-
mean(svm.test.predict.scaled != stdtest$V58)

# Log
## Linear
svm.train.log = svm(V58 ~., data = logtrain, kernal = "linear", scaled = False, type="C-classification")
svm.train.predict.log = predict(svm.train.log, logtrain)
svm.test.predict.log = predict(svm.train.log, logtest)

### Classification errors
linear_svm_error_log_train <- mean(svm.train.predict.log != logtrain$V58)
linear_svm_error_log_test <-
mean(svm.test.predict.log != logtest$V58)
...
# Standardized
## Non-Linear
svm.train.scaled = svm(V58 ~., data = stdtrain, kernel="polynomial",scaled = False, type="C-classification")
svm.train.predict.scaled = predict(svm.train.scaled, stdtrain)
svm.test.predict.scaled = predict(svm.train.scaled, stdtest)

### Classification errors
nonlinear_svm_error_scaled_train <- mean(svm.train.predict.scaled != stdtrain$V58)
nonlinear_svm_error_scaled_test <-
mean(svm.test.predict.scaled != stdtest$V58)

# Log
## Non-Linear
svm.train.log = svm(V58 ~., data = logtrain, kernel="polynomial",scaled = False, type="C-classification")
svm.train.predict.log = predict(svm.train.log, logtrain)
svm.test.predict.log = predict(svm.train.log, logtest)

### Classification errors
nonlinear_svm_error_log_train <- mean(svm.train.predict.log != logtrain$V58)
...
```
e) Apply tree-based classifiers to this data. What are the classification errors (training and test)? 
```{r}
# Standardized
train.scaled = data.frame(cbind(scale(train[, -58]), train[, 58]))
test.scaled = data.frame(cbind(scale(test[, -58]), test[, 58]))
